"""Opt-in Tailscale binding for the UI server.

``ui.sh start --tailscale`` binds this machine's Tailscale IP and serves
HTTPS with a cert ``tailscale cert`` issues for the node's MagicDNS name
(Let's Encrypt-backed, trusted by browsers with no manual setup) — the same
mechanism perceptua-nomon/nomothetic uses for its fleet API.

No fallback to a self-signed cert on failure: if Tailscale is not available,
the right answer is a clear error, not a quietly weaker cert.
"""
import json
import subprocess
...
def provision_cert(cert_dir: Path):
    """Issue a Tailscale cert for this node's MagicDNS name.

    Returns ``(cert_path, key_path, fqdn)``. Re-issues on every call —
    ``tailscale cert`` is cheap and idempotent, and the UI is a short-lived
    dev process, so there is no cache-and-check-expiry step to get wrong.
    """
    try:
