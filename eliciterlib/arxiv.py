...
here: query the arxiv Atom API, rank against your interest profile, and top up the reading
queue (`status.py`) to at most `ELICITER_ARXIV_KEEP` papers.

The consequence worth naming: relevance is defined by terms you stated and terms you have
written, and the digest says which ones matched. That is narrower than a model's judgement
of what you would like, and much more legible — you can see why a paper is there and tune
the profile until it is right.
...
def score(papers, db=None, keep=None, profile=None, log=print):
    """Rank papers by overlap with the interest profile. Returns **every** match, ranked.

    `keep` truncates, but defaults to no truncation, and the queue — not this function —
    is what enforces the ten-paper cap. Those must be different numbers: truncating here to
    the cap means that when you reject two papers, the next sweep has only ten candidates
    to offer and all ten are already seen, so the freed slots never refill. Observed
    exactly that way before this defaulted to unlimited.

    A paper matching nothing is dropped outright. That is the only threshold, and it is not
    a tunable one — "shares no vocabulary with anything you care about" is a fact about the
    paper, not a dial.
    """
    if not papers:
        return []
    prof = profile if profile is not None else build_profile(db, log=log)
    if not prof:
        log("[arxiv] no interests and no corpus — set ELICITER_INTERESTS in .env")
        return []

    scored = []
    for p in papers:
        sc, matched = rank.score(f"{p['title']}. {p['abstract']}", prof)
        if sc <= 0.0:
            continue
        scored.append(dict(p, score=round(sc, 4), matched=matched[:12]))

    scored.sort(key=lambda p: -p["score"])
    log(f"[arxiv] {len(scored)} of {len(papers)} paper(s) matched the profile")
    return scored[:keep] if keep else scored
...
def render_queue(queue, day=None, swept=None):
    ...
    L += [f"_{len(active)} paper(s) waiting, capped at {config.i('ELICITER_ARXIV_KEEP')}. "
          ...
          + "Generated by `scripts/arxiv-digest.sh`; the matched terms under each title "
          "are why it is here. Mark them with `scripts/papers.sh read|reject <id>` — a "
          "rejected paper never comes back, and a read one frees a slot._", ""]

    if not active:
        L += ["_Nothing waiting. Either the queue is clear, or nothing matched — widen "
              "`ELICITER_INTERESTS`, or run `scripts/arxiv-digest.sh --explain` to see "
              "what the profile actually contains._", ""]
    ...
    for i, p in enumerate(active, 1):
        ...
        L += [f"## {i}. {p['title']}",
              ...
              "",
              (f"**{float(p.get('score', 0)):.2f}** · matched {matched}" if matched
               else f"**{float(p.get('score', 0)):.2f}**"),
              "",
              p.get("abstract", ""),
              "",
              f"<{p.get('url', '')}>",
